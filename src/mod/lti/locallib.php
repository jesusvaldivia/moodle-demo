...
function lti_build_request($instance, $typeconfig, $course, $typeid = null, $islti2 = false) {
    ...
    if ($islti2 || $typeconfig['sendemailaddr'] == LTI_SETTING_ALWAYS ||
        ...
        if ($typeconfig['toolurl'] === "https://applications.zoom.us/lti/rich") {
            global $DB;
            if (is_siteadmin($USER)) {
                $query = "SELECT     u.id as id, email " .
                            "FROM       {course} c " .
                            "INNER JOIN {context} cx ON c.id = " . $course->id . " and c.id = cx.instanceid and cx.contextlevel = 50 " .
                            "INNER JOIN {role_assignments} ra ON cx.id = ra.contextid AND ra.roleid = 3 " .
                            "INNER JOIN {user} u ON ra.userid = u.id LIMIT 0,1 ";
                $resultado = $DB->get_record_sql($query);
                if ((!empty($resultado)) && (!empty($resultado->email)) ) {
                    $requestparams['lis_person_contact_email_primary'] = $resultado->email;
                }
            }
        }
    }
